...
// Опрос частый намеренно: реплики в чате разнесены по времени на несколько
// секунд, и при редком поллинге вся пауза съедалась бы задержкой опроса.
const POLL_MS = 1500;
const FEED_LIMIT = 300;

// Опрос состояния игры: прогресс, таблица утечек и лента одним запросом раз в
// три секунды. Лента приезжает инкрементально по курсору since, поэтому
// сообщения не задваиваются.
